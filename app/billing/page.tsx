...
export default function BillingPage() {
  ...
  const fetchTenantsWithBilling = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch('/api/tenants/with-billing', { headers: { 'Cache-Control': 'no-cache' } });
  ...
  const filteredTenants = useMemo(() => {
    return tenants.filter(tenant => {
      // Exclude moved-out tenants
      if (!tenant.is_active || tenant.move_out_date) return false;
              ...
              <Card key={tenant.id} className="card-elevated group hover:scale-[1.02] transition-all duration-200">
                ...
                <CardContent className="space-y-4">
                  ...
                  <Button 
                    ...
                    disabled={
                      // Allow if bill is overdue for creation (days_until_cycle_end < 0)
                      (!tenant.can_generate_bill && !(typeof tenant.days_until_cycle_end === 'number' && tenant.days_until_cycle_end < 0)) ||
                      (tenant.latest_bill && (
                        tenant.latest_bill.status === 'active' ||
                        tenant.latest_bill.status === 'partially_paid' ||
                        tenant.latest_bill.status === 'refund' ||
                        (typeof tenant.latest_bill.total_amount_due === 'number' && tenant.latest_bill.total_amount_due < 0)
                      ))
                    }
                  >
